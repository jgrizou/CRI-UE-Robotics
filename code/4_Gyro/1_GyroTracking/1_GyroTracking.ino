...
int offset_gyro;  // the gyro has a small offset at rest, we want to measure it and remove it from each reading we do
...
void setup()
{
  ...
  // Calibrate the gyro.
  // We just read the gyro 1000 times
  ...
  for (int i = 0; i < n_samples; i++)
  {
    ...
    // the logic is that is because this loop is going to be very fast to execute
    // we might read twice the same value as the gyro also need some time to upadte its measurement
    // wo we wait that a new value is available before reading it
  ...
  // we start at angle 0, this is arbitrary, 
  //we just say that whatever the robot angle is now, 
...
void loop()
{
  ...
  // change in angle is speed * dt and we need a convertion ratio from
  // the unit of the gyro to degrees
