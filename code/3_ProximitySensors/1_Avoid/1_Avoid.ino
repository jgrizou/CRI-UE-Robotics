 ...
 *  
 *  We reuse the previous code but only use one brightness level for the sensor to take our decision
 *  we choose the value from the previous 0_ReadProximity code
 *  
 *  In addition, we use the side proximity sensors of the RIGHT and LEFT side 
 *  to make the behavior of the robot more robust
 *  
...
// Proximity sensor PIN, all three of them
...
// NOTE: when we flash the left LED, both the front and side left LED turn on
// same for right
...
// we are using only the brightnessLevel of 2, 
// because we decided it triggers at a good distance for this task
...
void setup()
{
  ...
  // Setting pin mode of all proximity sensor to INPUT
...
void loop()
{
  ...
  // something ahead on my left side
  ...
  // something ahead on my right side
  else if (!front_sensor_left_led_state & front_sensor_right_led_state) {
    // turn left slightly
